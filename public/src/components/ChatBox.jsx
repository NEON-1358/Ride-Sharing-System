...
const ChatBox = ({ bookingId, rideOwnerId, rideOwnerName, passengerId, onClose }) => {
  ...
  const handleSend = (e) => {
    ...
    const toId = user.id === rideOwnerId ? passengerId : rideOwnerId;

    console.log("Sending message:", { room: bookingId, text: input, from: user.id, to: toId });
    socket.emit('message', {
      room: bookingId,
      text: input,
      from: user.id,
    ...
    <div className="chat-overlay">
      <div className="chat-box">
        <div className="chat-header">
          <div>
            <h3 className="text-white">Chat with {user.id === rideOwnerId ? 'Passenger' : rideOwnerName}</h3>
        ...
        <div className="chat-messages">
          ...
          {messages.map((msg, i) => (
            <div 
              key={i} 
              className={`msg ${msg.from === user.id ? 'msg-sent' : 'msg-received'}`}
            >
